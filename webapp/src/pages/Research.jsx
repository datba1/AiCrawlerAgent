  ...
  return (
    <div className="space-y-6 max-w-4xl animate-in fade-in duration-500">
      <div className="flex items-center space-x-3">
        ...
        <h1 className="text-xl font-semibold text-slate-900 tracking-tight">AI Research</h1>
      </div>

      <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
        <p className="text-slate-500 mb-4 text-xs">Enter a topic to generate a comprehensive research summary powered by AI agents.</p>
        <div className="flex flex-col md:flex-row gap-3">
          ...
          >
            {loading ? <Loader2 className="animate-spin" size={18} strokeWidth={1.5} /> : <span>Research</span>}
          </button>
        </div>
      </div>

      {error && (
        <div className="bg-rose-50 border border-rose-100 text-rose-700 p-3 rounded-lg flex items-start space-x-3 animate-in slide-in-from-top-2">
          <AlertCircle className="mt-0.5 flex-shrink-0" size={16} strokeWidth={1.5} />
      ...
      {result && (
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm animate-in zoom-in-95 duration-300">
          <h2 className="text-lg font-semibold text-slate-800 mb-4 border-b border-slate-100 pb-4">
